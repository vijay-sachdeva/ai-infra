#!/usr/bin/env python3
"""Regenerate eu/data/projects.csv and eu/data/projects.geojson from eu/data/projects.json.

projects.json is the canonical source; these flat/geo exports are derived. Run after
editing the ledger:  python scripts/eu/build_projects_exports.py
Stdlib-only (no deps), so it runs anywhere / in CI. Mirrors the US
scripts/build_projects_exports.py, with EU extras (country_name, record_type).
"""
import csv, io, json, os

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
SRC = os.path.join(ROOT, "eu", "data", "projects.json")

COLS = ["id", "name", "operator", "state", "country_name", "record_type",
        "lat", "lon", "coord_precision", "capacity_mw", "capacity_type",
        "status", "status_as_of", "power_generation", "power_model",
        "provenance", "transformation", "confidence",
        "primary_source_url", "primary_source_publisher"]
...
def main():
    ...
    with io.open(os.path.join(ROOT, "eu", "data", "projects.csv"), "w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(COLS)
        for r in recs:
            ...
            w.writerow([r["id"], r["name"], r["operator"], r["state"],
                        r.get("country_name", ""), r.get("record_type", ""),
    ...
    for r in recs:
        ...
        feats.append({"type": "Feature",
                      ...
                      "properties": {"id": r["id"], "name": r["name"], "operator": r["operator"],
                                     "state": r["state"], "country_name": r.get("country_name"),
                                     ...
                                     "source_url": url}})
    gj = {"type": "FeatureCollection", "name": "Europe AI data-center projects",
          "license": "CC-BY-4.0", "attribution": d.get("attribution", ""), "features": feats}
    open(os.path.join(ROOT, "eu", "data", "projects.geojson"), "w", encoding="utf-8").write(
